<?php
    class SubjectMap extends BaseMap {
        ...
        public function findById($id = null) {
            if ($id) {
                $res = $this->db->query("SELECT subject_id, name, otdel_id, hours "
        ...
        private function update(Subject $subject) {
            $name = $this->db->quote($subject->name);
            if ( $this->db->exec("UPDATE subject SET name = $name, otdel_id = $subject->otdel_id, hours = $subject->hours WHERE subject_id = ".$subject->subject_id) == 1) {
                return true;
            }
            return false;
        }
        ...
        public function findViewById($id = null) {
            if ($id) {
                $res = $this->db->query("SELECT subject.subject_id, subject.name, otdel.name AS otdel, subject.hours" . " FROM subject INNER JOIN otdel ON subject.otdel_id=otdel.otdel_id WHERE subject_id = $id");
