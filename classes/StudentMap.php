<?php
    class StudentMap extends BaseMap {
        ...
        private function insert(Student $student) {
            if ($this->db->exec("INSERT INTO student(user_id, gruppa_id, num_zach) 
                                VALUES($student->user_id, $student->gruppa_id, $student->num_zach)") == 1) {
                return true;
            }
            return false;
        }
        ...
        public function findAll($ofset = 0, $limit = 30) {
            $res = $this->db->query("SELECT user.user_id, CONCAT(user.lastname,' ', user.firstname, ' ', user.patronymic) AS fio, user.birthday, "
                                    . " gender.name AS gender, gruppa.name AS gruppa, student.num_zach, role.name AS role FROM user INNER JOIN student ON user.user_id=student.user_id "
        ...
        public function findProfileById($id = null) {
            if ($id) {
                ...
                gruppa.name AS gruppa, num_zach FROM student "
